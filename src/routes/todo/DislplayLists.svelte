<script lang="ts">
  ...
  export const typedListStore = readable(listArr, (set) => {
  ...
  export const typedCurrentListStore = writable(currentList);
  ...
  const DeleteList = (itemIdToDelete: number) => {
    ...
    let idd = 1;
    typedCurrentListStore.update(() => idd);
  };

  const SetCurrentList = (itemId: number) => {
    typedCurrentListStore.update(() => itemId);
  };
</script>

<div class="flex flex-col mt-4">
  {#each $typedListStore as item (item.id)}
    <div class="flex mb-2">
      <div
        ...
        class:bg-gray-800={$typedCurrentListStore === item.id}
      >
